...
const BLOCKLIST_WORDS = [
    'audio', 'bio', 'biology', 'chemical', 'chemistry', 'drug',
    'memo', 'modal', 'molecule', 'mri', 'protein', 'speech',
    'video', 'vision', 'visual', 'x-ray',
    'basque', 'chinese', 'french', 'greek', 'hindi', 'indian',
    'japanese', 'portuguese', 'romanian', 'spanish'
];
...
function displayPapers(papers) {
    ...
    highlightedContainer.innerHTML = '<h2>Highlighted Papers</h2>' +
        highlightedPapers.map((p, i) => createPaperElement(p, i, true)).join('');

    // Display other papers
    const otherContainer = document.getElementById('other-papers');
    otherContainer.innerHTML = '<h2>Other Papers</h2>' +
        otherPapers.map((p, i) => createPaperElement(p, i, false)).join('');
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('process-content').addEventListener('click', () => {
        ...
        try {
            ...
            document.getElementById('error').textContent = 'Failed to process content';
